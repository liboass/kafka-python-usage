...
def thread_producer():
    try:
        ...
        for i in range(100):
            print(f"send msg id {i + 10}")
            producer.send(TOPIC, value=f"message id:{i + 10}".encode('utf-8'))
            time.sleep(1)
        producer.flush()
        producer.close()
    except Exception:
...
def thread_consumer(consumer_id=0):
    try:
        consumer = KafkaConsumer(group_id=TOPIC, bootstrap_servers=['localhost:9092'], auto_offset_reset='earliest', enable_auto_commit=False)
